...
class NaiveBayesClassifier:
    ...
    def predict(self, input_dict):
        ...
        for n in range(len(self.__unique_values)):
            posterior_probability_result[n] = self.__compute(input_dict, n, need_smoothing)
    ...
    def __compute(self, input_dict, item_index, need_smoothing):
        ...
        for data_key in col_list:
            ...
            denominator *= compute_op(sum(current_iter_data), entry_count, self.__alpha, k_value)
        ...
        for data_key in col_list:
            input_key = input_dict[data_key]
            current_iter_data = self.__data_frequency[data_key][input_key]
            numerator *= compute_op(current_iter_data[item_index], occurence_count, self.__alpha, k_value) 
